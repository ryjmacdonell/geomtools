...
def test_translate_ax_all():
    ...
    assert np.all(np.isclose(diff_xyz, np.ones((6, 3))))
...
def test_translate_x_carbon():
    ...
    soln = np.zeros((6, 3))
    soln[:2,0] = 1.
    assert np.all(np.isclose(diff_xyz, soln))
...
def test_rotmat_zero():
    ...
    assert np.all(np.isclose(rot, np.eye(3)))
...
def test_rotmat_degrees():
    ...
    soln[0,2] = -1.
    assert np.all(np.isclose(rot, soln))
...
def test_angax_identity():
    ...
    assert np.isclose(ang, 0.) and np.isclose(d, 1.)


def test_angax_invert():
    ang, ax, d = displace.angax(-np.eye(3))
    assert np.isclose(ang, np.pi) and np.isclose(d, -1.)
...
def test_angax_pi():
    ...
    assert np.isclose(ang, np.pi) and np.all(np.isclose(ax, [0, 1, 0]))
...
def test_angax_degrees():
    ...
    assert np.isclose(ang, 90.) and np.all(np.isclose(ax, [0, 1, 0]))
...
def test_rotate_degrees():
    ...
    assert np.all(np.isclose(new_xyz, soln))


def test_rotate_invert():
    new_xyz = displace.rotate(c2h4[1], np.pi, 'z', det=-1)
    assert np.all(np.isclose(new_xyz, -c2h4[1]))
...
def test_get_centremass_molecule():
    ...
    assert np.all(np.isclose(cm, np.ones(3)))


def test_get_centremass_atom():
    cm = displace.get_centremass(c2h4[0][0], c2h4[1][0])
    assert np.all(np.isclose(cm, c2h4[1][0]))
